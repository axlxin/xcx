...
class ControllerExtensionModuleMailchimpIntegration extends Controller {
	...
	public function index() {
		...
		$data['settings'][] = array(
			'type'		=> 'tabs',
			'tabs'		=> array('extension_settings', 'list_settings', 'merge_tags', 'interest_groups', 'ecommerce', 'module_settings', 'testing_mode'),
		);
		$data['settings'][] = array(
			'type'		=> 'html',
		...
		$data['settings'][] = array(
			'type'		=> 'html',
			'content'	=> '<div class="text-info text-center">' . $data['help_merge_tags'] . '</div>',
		);
		$data['settings'][] = array(
			'key'		=> 'merge_tags',
			'type'		=> 'heading',
				...
				<script type="text/javascript">
					function syncOrders() {
						if (confirm("' . $data['text_sync_note'] . '")) {
							$("#syncing").fadeIn();
							$.ajax({
								url: "index.php?route=extension/' . $this->type . '/' . $this->name . '/syncOrders&token=' . $data['token'] . '&start=" + $("#starting-order-id").val() + "&end=" + $("#ending-order-id").val(),
								success: function(data) {
									alert(data);
									$("#syncing").fadeOut();
								},
