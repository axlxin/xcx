<?php
require_once(substr_replace(DIR_SYSTEM, '', -7) . 'vendor/equotix/canned_messages/equotix.php');
class ControllerModuleCannedMessages extends Equotix {
	...
	public function index() {
		$this->language->load('module/canned_messages');
		...
		$data['button_cancel'] = $this->language->get('button_cancel');
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/canned_messages', 'token=' . $this->session->data['token'], 'SSL')
   		);
        
        $this->load->model('module/canned_messages');
        
        $messages = $this->model_module_canned_messages->getMessages();
		...
		$this->generateOutput('module/canned_messages.tpl', $data);
	}
    ...
    public function add() {
        if ($this->validate()) {
            ...
            $this->model_module_canned_messages->addMessage($this->request->post);
        }
    ...
    public function edit() {
        if ($this->validate() && isset($this->request->get['canned_message_id'])) {
            ...
            $this->model_module_canned_messages->editMessage($this->request->get['canned_message_id'], $this->request->post);
        }
    ...
    public function delete() {
        if ($this->validate() && isset($this->request->get['canned_message_id'])) {
            ...
            $this->model_module_canned_messages->deleteMessage($this->request->get['canned_message_id']);
        }
    ...
    public function info() {
        ...
        $json = $this->model_module_canned_messages->getMessage($this->request->get['canned_message_id']);
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/canned_messages') || !$this->validated()) {
