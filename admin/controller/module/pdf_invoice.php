...
class ControllerModulePdfInvoice extends Controller {
  ...
  public function __construct($registry) {
    ...
    if (defined('JPATH_MIJOSHOP_OC') && !$this->OC_V2) {
  	...
  	$data['OC_V2'] = $this->OC_V2;
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if ($this->OC_V2) {
				$this->response->redirect($this->url->link('module/pdf_invoice', 'token=' . $this->session->data['token'] . $redirect_store, 'SSL'));
			} else {
				$this->redirect($this->url->link('module/pdf_invoice', 'token=' . $this->session->data['token'] . $redirect_store, 'SSL'));
			}
		}
		
		if (is_file(DIR_SYSTEM.'../vqmod/xml/pdf_invoice_pro.xml')) {
			$data['module_version'] = simplexml_load_file(DIR_SYSTEM.'../vqmod/xml/pdf_invoice_pro.xml')->version;
      $data['module_type'] = 'vqmod';
		} else if (is_file(DIR_SYSTEM.'../system/pdf_invoice_pro.ocmod.xml')) {
      $data['module_version'] = simplexml_load_file(DIR_SYSTEM.'../system/pdf_invoice_pro.ocmod.xml')->version;
      ...
      $data['module_type'] = '';
		...
		// module checks
    if (is_file(DIR_SYSTEM.'../vqmod/xml/pdf_invoice_pro.xml') && is_file(DIR_SYSTEM.'../system/pdf_invoice_pro.ocmod.xml')) {
      $this->error['warning'] = 'Warning : both vqmod and ocmod version are installed<br/>- delete /vqmod/xml/pdf_invoice_pro.xml if you want to use ocmod version<br/>- or delete /system/pdf_invoice_pro.ocmod.xml if you want to use vqmod version';
    }
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pdf_invoice', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('module/pdf_invoice', 'token=' . $this->session->data['token'] . '&store_id=' . $store_id, 'SSL');
    ...
    if (isset($this->request->post['pdf_invoice_barcode'])) {
		...
		// columns
		$default_columns = array('product_id', 'image', 'product', 'sku', 'upc', 'ean', 'mpn', 'isbn', 'model', 'weight', 'quantity', 'price', 'price_tax', 'tax', 'tax_total', 'tax_rate', 'total');
    ...
    if (isset($this->request->post['pdf_invoice_slip_barcode'])) {
		...
		// columns
		$default_slip_columns = array('image', 'product', 'model', 'manufacturer', 'mpn', 'location', 'sku', 'upc', 'ean', 'weight', 'quantity', 'slip_qty', 'qty_check', 'expected');
			...
			if ( count($files) > 2 ) { /* . and .. */
				...
				foreach( $files as $file ) {
					if ( file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' && !is_dir($root . $_POST['dir'] . $file) && substr($file, -3) == 'pdf' ) {
						...
						echo '<li class="file ext_'.$ext.'"><a target="new" href="'.$this->url->link('module/pdf_invoice/getfile', 'dir=' . htmlentities($_POST['dir'])  . '&file=' . htmlentities($file) . '&token=' . $this->session->data['token'], 'SSL').'" rel="' . htmlentities($_POST['dir'] . $file) . '">' . htmlentities($file) . '</a></li>';
					}
				}
				echo "</ul>";	
			}
  ...
  public function modal_info() {
	...
	public function install() {
		...
		//$this->redirect($this->url->link('module/pdf_invoice', 'token=' . $this->session->data['token'], 'SSL'));
	}
