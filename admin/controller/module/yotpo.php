	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL'));
				...
				$data['yotpo_error_warning'] = $result['message'];
			}
		}
		...
		foreach (self::$error_assigns as $error_assign) {
			if (isset($this->error[$error_assign])) {
				$data[$error_assign] = $this->error[$error_assign];
			} else {
				$data[$error_assign] = '';
			}
		}
		...
		foreach (self::$config_assigns as $config_assign) {
			if (isset($this->request->post[$config_assign])) {
				$data[$config_assign] = $this->request->post[$config_assign];
			} else {
				$data[$config_assign] = $this->config->get($config_assign);
			}
		}

		/* Default widget tab name: 'Reviews', Default widget tab location: footer Default widget language: english*/
		if(is_null($data['yotpo_review_tab_name']) || $data['yotpo_review_tab_name'] == '') {
			$data['yotpo_review_tab_name'] = $this->language->get('yotpo_default_reviews_tab_name');
		}

		if(is_null($data['yotpo_widget_location']) || $data['yotpo_widget_location'] == '') {
			$data['yotpo_widget_location'] = $this->language->get('yotpo_default_widget_location');
		}

		if(is_null($data['yotpo_language']) || $data['yotpo_language'] == '') {
			$data['yotpo_language'] = $this->language->get('yotpo_default_widget_language');
		}

		if(is_null($data['yotpo_map_status']) || $data['yotpo_map_status'] == '') {
			$data['yotpo_map_status'] = array();
			array_push($data['yotpo_map_status'], $this->config->get('config_complete_status_id'));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/yotpo', 'token=' . $this->session->data['token'], 'SSL'),
      		...
      		$data['action'] = $this->url->link('module/yotpo', 'token=' . $this->session->data['token'], 'SSL');

      		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

      		$data['sign_up'] = $this->url->link('module/yotpo', 'token=' . $this->session->data['token'] . '&action=signup', 'SSL');

      		$data['past_orders'] = $this->url->link('module/yotpo', 'token=' . $this->session->data['token'] . '&action=past_orders', 'SSL');
      		$past_orders_enable = $this->config->get('yotpo_past_order_sent');

      		$data['yotpo_show_past_orders_button'] = empty($past_orders_enable) ? true : false;

      		$data['yotpo_show_dashborad_link'] = empty($data['yotpo_appkey']) || empty($data['yotpo_secret']) ? false : true;
      		if($data['yotpo_show_dashborad_link']) {
      			$data['yotpo_dashborad_link'] = 'https://api.yotpo.com/users/b2blogin?app_key='.$data['yotpo_appkey'].'&secret='.$data['yotpo_secret'];
      			$data['yotpo_dashborad_link_text'] = $this->language->get('text_customize_widget');
      			$data['yotpo_dashborad_text'] = $this->language->get('text_yotpo_dashboard');
      		}
      		else {
      			$data['text_yotpo_missing_app_key'] = $this->language->get('text_yotpo_missing_app_key');
      			$data['text_yotpo_log_in'] = $this->language->get('text_yotpo_log_in');
      			$data['yotpo_login_link'] = 'https://www.yotpo.com/?login=true';
      		}
      		if($data['yotpo_widget_location'] == 'other') {
      			$data['text_yotpo_widget_location_other'] = $this->language->get('text_yotpo_widget_location_other');
      		}
			...
			$this->response->setOutput($this->load->view('module/yotpo.tpl', $data));
      		//Send the output.
