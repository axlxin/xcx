...
class ModelJournal2Blog extends Model{
    ...
    public function post() {
        ...
        $query7 = $this->db->query("
            SELECT
                p.related_post_id as related_post_id,
                pd.name as name
            FROM `{$this->db_prefix}journal2_blog_post_to_post` p
            LEFT JOIN `{$this->db_prefix}journal2_blog_post_description` pd on pd.post_id = p.related_post_id
            WHERE p.post_id = {$post_id} AND pd.language_id = {$this->language_id}
        ");
        ...
        foreach ($query2->rows as $row) {
            $result['name']['value'][$row['language_id']] = $row['name'];
            $result['description'][$row['language_id']] = $row['description'];
            $result['short_description']['value'][$row['language_id']] = $row['short_description'];
            $result['meta_title']['value'][$row['language_id']] = $row['meta_title'];
            $result['meta_keywords']['value'][$row['language_id']] = $row['meta_keywords'];
            $result['meta_description']['value'][$row['language_id']] = $row['meta_description'];
            $result['keyword']['value'][$row['language_id']] = $row['keyword'];
    ...
    public function edit_post() {
        ...
        $sort_order = (int)$this->db->escape(Journal2Utils::getProperty($this->post_data, 'sort_order', ''));
        $author_id = (int)$this->db->escape(Journal2Utils::getProperty($this->post_data, 'author_id', ''));
        $keyword = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'keyword.value.' , ''));
        $this->db->query("
        ...
        $this->db->query("
            UPDATE `{$this->db_prefix}url_alias` SET
                keyword = '{$keyword}'
            WHERE query ='post_id={$post_id}' 
        ");   //新增代码，更新oc_url_alias表字段内容
    ...
    private function edit_post_data($post_id) {
        ...
        $this->db->query("DELETE FROM `{$this->db_prefix}journal2_blog_post_to_post` WHERE post_id = {$post_id}");
        $this->db->query("DELETE FROM `{$this->db_prefix}url_alias` WHERE query = 'post_id={$post_id}'"); //新增代码，删除语句
        $languages = $this->model_localisation_language->getLanguages();
        ...
        foreach ($languages as $language) {
            ...
            $name = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'name.value.' . $language['language_id'], ''));
            $description = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'description.' . $language['language_id'], ''));
            $short_description = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'short_description.value.' . $language['language_id'], ''));
            $meta_title = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'meta_title.value.' . $language['language_id'], ''));
            $meta_keywords = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'meta_keywords.value.' . $language['language_id'], ''));
            $meta_description = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'meta_description.value.' . $language['language_id'], ''));
            $keyword = $this->db->escape(Journal2Utils::getProperty($this->post_data, 'keyword.value.' . $language['language_id'], ''));
            ...
            $this->db->query("
                INSERT INTO `{$this->db_prefix}journal2_blog_post_description`
                    (post_id, language_id, name, description, short_description, meta_title, meta_keywords, meta_description, keyword, tags)
                VALUES
                    ({$post_id}, {$language_id}, '{$name}', '{$description}', '{$short_description}', '{$meta_title}', '{$meta_keywords}', '{$meta_description}', '{$keyword}', '{$tags}')
            ");
            ...
            $this->db->query("
             INSERT INTO `{$this->db_prefix}url_alias`
             (query,keyword,language_id)values ('post_id={$post_id}','blog/{$keyword}',{$language_id})
            "); //新增代码，写入oc_url_alias表中
        }
        ...
        /* related posts */
        $related_posts = array();
        foreach (Journal2Utils::getProperty($this->post_data, 'related_posts', array()) as $related_post) {
            if ($related_post_id = Journal2Utils::getProperty($related_post, 'data.id')) {
                $related_posts[] = (int)$related_post_id;
            }
        }
        ...
        foreach ($related_posts as $related_post_id) {
            $this->db->query("
                INSERT INTO `{$this->db_prefix}journal2_blog_post_to_post`
    ...
    public function delete_post() {
        ...
        $this->db->query("DELETE FROM `{$this->db_prefix}url_alias` WHERE query = 'post_id={$post_id}'");
        return null;
    }
