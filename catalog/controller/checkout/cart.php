...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {//判断购物车是否有商品
			...
			if ($this->config->get('config_customer_price') && !$this->customer->isLogged()) { //客户是否有登录
			...
			if ($this->config->get('config_cart_weight')) { //购物车是否有商品
			...
			$products = $this->cart->getProducts();  //拿到购物车的商品信息

			foreach ($products as $product) {
				$product_total = 0;

				foreach ($products as $product_2) {
					if ($product_2['product_id'] == $product['product_id']) { //验证商品信息
				...
				foreach ($product['option'] as $option) {
					...
					$this->response->setOutput($this->load->view('journal2/cart.tpl',$data));
				}
			...
			if (!$json) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					...
					array_multisort($sort_order, SORT_ASC, $total_data);
                                ...
                                $json['cart_qty'] = $this->cart->countProducts();
