...
class ControllerJournal2Search extends Controller {
    ...
    public function index() {
        ...
        if(isset($this->request->get['search'])) {
            $search_autocomplete_include_description = $this->journal2->settings->get('search_autocomplete_include_description', '1');
            $results = $this->model_journal2_search->search($this->request->get['search'], $this->journal2->settings->get('autosuggest_limit', 0), $search_autocomplete_include_description === '1', true);
            ...
            if ($json['results']) {
                ...
                if($search_autocomplete_include_description === '1'){
                    $param = "&description=true";
                }
                
                if (VERSION === '1.5.4' || VERSION === '1.5.4.1') {
                    $json['view_more_url'] = $this->url->link('product/search', $param.'&filter_name=' . urlencode(html_entity_decode($this->request->get['search'], ENT_QUOTES, 'UTF-8')));
                } else {
                    $json['view_more_url'] = $this->url->link('product/search', $param.'&search=' . urlencode(html_entity_decode($this->request->get['search'], ENT_QUOTES, 'UTF-8')));
                }
            } else {
