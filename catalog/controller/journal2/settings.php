...
class ControllerJournal2Settings extends Controller {
    ...
    public function index() {
        ...
        $html .= '<a class="button notification-cart" href="' . $this->url->link('checkout/cart', '', $this->config->get('config_secure')) . '">' . trim(preg_replace('/\s+/', ' ', $this->language->get('text_cart'))) . '</a>';
        $html .= '<a class="button notification-checkout" href="' . $this->url->link('checkout/checkout', '', 'SSL') . '">' . trim(preg_replace('/\s+/', ' ', $this->language->get('text_checkout'))) . '</a>';
        $html .= '</div>';
    ...
    public function sitemap() {
        ...
        foreach ($categories as $category) {
            $blog_categories[] = array(
                'name'  => $category['name'],
                'href'  => $this->url->link('journal2/blog', 'journal_blog_category_id=' . $category['category_id'], $this->config->get('config_secure'))
            );
        }
        ...
        $this->journal2->settings->set('blog_href', $this->url->link('journal2/blog', '', $this->config->get('config_secure')));
