...
class ControllerModuleJournal2SuperFilter extends Controller {
    ...
    public function products() {
        ...
        $this->data['compare'] = $this->url->link('product/compare', '', $this->config->get('config_secure'));
        ...
        foreach ($results as $result) {
            ...
            switch ($data['route']) {
                case 'product/category':
                    $href = $this->url->link('product/product', 'path=' . $data['path'] . '&product_id=' . $result['product_id'] . $url, $this->config->get('config_secure'));
                    break;
                case 'product/manufacturer':
                    $href = $this->url->link('product/product', '&manufacturer_id=' . $data['manufacturer_id'] . '&product_id=' . $result['product_id'] . $url, $this->config->get('config_secure'));
                    break;
                default :
                    $href = $this->url->link('product/product', 'product_id=' . $result['product_id'] . $url, $this->config->get('config_secure'));
                    break;
            }
        ...
        switch ($data['route']) {
            case 'product/category':
                ...
                $pagination->url = $this->url->link('product/category', 'path=' . $data['path'] . $url . '&page={page}', $this->config->get('config_secure'));
                break;
            case 'product/manufacturer/info':
                $route = 'product/manufacturer/info';
                $pagination->url = $this->url->link('product/manufacturer/info','manufacturer_id=' . $data['manufacturer_id'] .  $url . '&page={page}', $this->config->get('config_secure'));
                break;
            case 'product/search':
                $route = 'product/search';
                $pagination->url = $this->url->link('product/search', $url . '&page={page}', $this->config->get('config_secure'));
                break;
            case 'product/special':
                $route = 'product/special';
                $pagination->url = $this->url->link('product/special', $url . '&page={page}', $this->config->get('config_secure'));
                break;
            default:
                $route = '';
        }
        ...
        $this->data['sorts'][] = array(
            ...
            'href'  => $this->url->link($route, 'sort=p.sort_order&order=ASC' . $url, $this->config->get('config_secure'))
        );

        $this->data['sorts'][] = array(
            'text'  => $this->language->get('text_name_asc'),
            'value' => 'pd.name-ASC',
            'href'  => $this->url->link($route, 'sort=pd.name&order=ASC' . $url, $this->config->get('config_secure'))
        );

        $this->data['sorts'][] = array(
            'text'  => $this->language->get('text_name_desc'),
            'value' => 'pd.name-DESC',
            'href'  => $this->url->link($route, 'sort=pd.name&order=DESC' . $url, $this->config->get('config_secure'))
        );

        $this->data['sorts'][] = array(
            'text'  => $this->language->get('text_price_asc'),
            'value' => 'p.price-ASC',
            'href'  => $this->url->link($route, 'sort=p.price&order=ASC' . $url, $this->config->get('config_secure'))
        );

        $this->data['sorts'][] = array(
            'text'  => $this->language->get('text_price_desc'),
            'value' => 'p.price-DESC',
            'href'  => $this->url->link($route, 'sort=p.price&order=DESC' . $url, $this->config->get('config_secure'))
        );
        ...
        if ($this->config->get('config_review_status')) {
            $this->data['sorts'][] = array(
                ...
                'value' => 'rating-DESC',
                'href'  => $this->url->link($route, 'sort=rating&order=DESC' . $url, $this->config->get('config_secure'))
            );

            $this->data['sorts'][] = array(
                'text'  => $this->language->get('text_rating_asc'),
                'value' => 'rating-ASC',
                'href'  => $this->url->link($route, 'sort=rating&order=ASC' . $url, $this->config->get('config_secure'))
            );
        }
        ...
        $this->data['sorts'][] = array(
            ...
            'value' => 'p.model-ASC',
            'href'  => $this->url->link($route, 'sort=p.model&order=ASC' . $url, $this->config->get('config_secure'))
        );

        $this->data['sorts'][] = array(
            'text'  => $this->language->get('text_model_desc'),
            'value' => 'p.model-DESC',
            'href'  => $this->url->link($route, 'sort=p.model&order=DESC' . $url, $this->config->get('config_secure'))
        );
        ...
        foreach($limits as $limit){
            $this->data['limits'][] = array(
                'text'  => $limit,
                'value' => $limit,
                'href'  => $this->url->link($route, $url . '&limit=' . $limit, $this->config->get('config_secure'))
            );
        }
