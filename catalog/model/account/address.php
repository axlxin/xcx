...
class ModelAccountAddress extends Model {
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			$customer_query = $this->db->query("SELECT * FROM `" . DB_PREFIX ."customer` WHERE customer_id='" . (int)$address_query->row['customer_id'] . "'");
            
			if($customer_query->num_rows)
			{
				$customer_telephone = $customer_query->row['telephone'];
			}
			else
			{
				$customer_telephone = '';
