...
class ModelJournal2Blog extends Model {
    ...
    public function getPostRelatedPosts($post_id, $limit = 5) {
        $post_id = (int)$post_id;
        
        $query = $this->db->query("SELECT related_post_id FROM `{$this->db_prefix}journal2_blog_post_to_post` WHERE post_id = {$post_id}");
        ...
        foreach ($query->rows as $row) {
            if($index<$limit){
                $result = $this->getPost($row['related_post_id']);
                
                if(!empty($result)){
                    $result['date'] = isset($result['date_created']) ? $result['date_created'] : "";
                    $results[] = $result;
                }
            }else{
                break;
