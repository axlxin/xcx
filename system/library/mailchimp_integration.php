<?php
//==============================================================================
// MailChimp Integration Pro v302.2
// 
...
class Mailchimp_Integration {
	...
	public function __construct($config, $db, $session, $url) {
		$this->config = $config;
		$this->db = $db;
	...
	public function send($data) {
		...
		if (empty($settings['status'])) {
			if ($settings['testing_mode']) $this->logMessage(strtoupper($this->name) . ' ERROR: Extension is disabled');
			return;
		}
		if (empty($settings['apikey'])) {
			if ($settings['testing_mode']) $this->logMessage(strtoupper($this->name) . ' ERROR: API Key is not filled');
			return;
		}
		if (empty($settings['listid'])) {
			if ($settings['testing_mode']) $this->logMessage(strtoupper($this->name) . ' ERROR: Default list is not set');
			return;
		}
	...
	public function sync($start, $end) {
		...
		if ($settings['autocreate']) {
			...
			foreach ($lists as $list) {
				...
				foreach ($diff_emails as $email) {
					...
					if (!empty($response['error'])) {
						if ($settings['testing_mode']) $this->logMessage(strtoupper($this->name) . ' SYNC ERROR: ' . $errors);
		...
		if ($settings['testing_mode']) {
			$this->logMessage(strtoupper($this->name) . ' SYNC SUCCESS: ' . str_replace("\n", ' ', $output));
		}
	...
	public function sendCart($cart, $customer_id) {
		...
		if (!empty($response['error']) && $settings['testing_mode']) {
			$this->logMessage(strtoupper($this->name) . ' CART ERROR: ' . $response['error']);
		}
	}
	...
	private function buildLineItems($products, $mailchimp_store_id, &$operations, &$lines) {
		...
		foreach ($products as $product) {
			...
			$seo_url_query = $this->db->query("SELECT * FROM " . DB_PREFIX . $seo_table . " WHERE `query` = 'product_id=" . (int)$product['product_id'] . "'");
	...
	public function formatOrder($order_info, $ordertype) {
		...
		if (!in_array($order_info['order_status_id'], explode(';', $settings['orderstatus']))) {
			if ($settings['testing_mode']) {
				$this->logMessage(strtoupper($this->name) . ': order ID ' . $order_info['order_id'] . ' was not sent to MailChimp because its order status is ' . $order_status);
			}
			return array();
		}
		...
		*/
		...
		if (empty($curl_data['lines'])) {
			if ($settings['testing_mode']) {
				$this->logMessage(strtoupper($this->name) . ': No valid products on order #' . $order_info['order_id']);
			}
			return array();
		}
	...
	public function syncOrders($start, $end) {
		...
		if ($settings['testing_mode']) {
			$this->logMessage(strtoupper($this->name) . ' SYNC SUCCESS: ' . $output);
		}
	...
	public function webhook($type, $data) {
		...
		if (empty($settings['status'])) {
			if ($settings['testing_mode']) {
				$this->logMessage(strtoupper($this->name) . ' ERROR: Extension is disabled in the admin panel');
			}
			return;
		}
		if (empty($settings['webhooks'])) {
			if ($settings['testing_mode']) {
				$this->logMessage(strtoupper($this->name) . ' ERROR: No webhooks are enabled in the admin panel');
			}
			return;
		}
		...
		*/
		...
		if ($type == 'subscribe' && in_array('subscribe', $webhooks)) {
			...
			if (empty($customer_query->row['address_id'])) {
				$this->logMessage(strtoupper($this->name) . ' WEBHOOK ERROR: customer ' . $data['email'] . ' does not have a valid address_id');
				return;
			}
		...
		if ($settings['testing_mode'] && $success) {
			$this->logMessage(strtoupper($this->name) . ' WEBHOOK SUCCESS: ' . $type . ' ' . $data['email'] . ' (List ID ' . $data['list_id'] . ')');
		}
	}
	...
	private function createCustomer($data) {
		...
		if ($settings['testing_mode']) {
			$this->logMessage(strtoupper($this->name) . ' CUSTOMER CREATED: ' . $customer['firstname'] . ' ' . $customer['lastname'] . ' (' . $customer['email'] . ')');
		}
	}
	...
	private function ruleViolation($rules, $rule, $value) {
		...
		$prefix = (version_compare(VERSION, '3.0', '<')) ? '' : $this->type . '_';
		if ($this->config->get($prefix . $this->name . '_testing_mode') && $violation) {
			$this->logMessage(strtoupper($this->name) . ': Mapping for list ID ' . $rules['list'] . ' ignored due to violating rule "' . $rule . ' ' . $comparison . ' ' . implode(', ', $rules[$rule][$comparison]) . '" with value "' . (is_array($value) ? implode(',', $value) : $value) . '"');
		}
	...
	private function inRange($value, $range_list, $charge_type = '', $list) {
		...
		$prefix = (version_compare(VERSION, '3.0', '<')) ? '' : $this->type . '_';
		if ($this->config->get($prefix . $this->name . '_testing_mode') && (strpos($charge_type, ' not') ? $in_range : !$in_range)) {
			$this->logMessage(strtoupper($this->name) . ': mapping for list ID ' . $list . ' ignored for violating rule "' . $charge_type . (strpos($charge_type, ' not') ? ' is not ' : ' is ') . implode(', ', $range_list) . '" with value "' . $value . '"');
		}
	...
	private function curlRequest($request, $api, $data = array()) {
		...
		curl_setopt($curl, CURLOPT_USERPWD, ' :' . $this->config->get($prefix . $this->name . '_apikey'));
		$response = json_decode(curl_exec($curl), true);
		
		if ($this->config->get($prefix . $this->name . '_testing_mode') == 'debug') {
			$hr = '------------------------------------------------------------------------------';
			$this->logMessage($hr . "\n" . $request . '   ' . $url . $api . "\n" . $hr);
		...
		if (!empty($response['error']) && $this->config->get($prefix . $this->name . '_testing_mode')) {
			$this->logMessage(strtoupper($this->name) . ' ERROR: ' . $response['error']);
		}
		return $response;
	}
}
