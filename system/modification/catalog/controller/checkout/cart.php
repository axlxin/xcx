    ...
    foreach($tmpAllOptions as $tmpAllOption){
        if(in_array($tmpAllOption["product_option_id"], $filter_product_option_ids)){
            ...
            if(count($tmpAllOption["product_option_value"])>0){
              ...
              var_dump($productAllOptions[$product_option_id]);
             ...
             if(isset($productAllOptions[$option['product_option_id']]) && $productAllOptions[$option['product_option_id']]){
				 //var_dump($productAllOptions[$option['product_option_id']]);
...
// Clear Thinking: MailChimp Integration Pro
				...
				if ($this->config->get($prefix . 'mailchimp_integration_sendcarts') && $this->customer->isLogged()) {
					...
					$mailchimp_integration->sendCart($this->cart, $this->customer->getId());
				}
				// end
