...
class ControllerJournal2Blog extends Controller {
    ...
    public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('common/home', '', $this->config->get('config_secure')),
            'separator' => false
        );
        ...
        if ($category_info) {
            ...
            $this->data['breadcrumbs'][] = array(
                ...
                'href'      => $this->url->link('journal2/blog', 'journal_blog_category_id=' . $category_id . $url, $this->config->get('config_secure')),
                'separator' => $this->language->get('text_separator')
            );
        ...
        if ($this->journal2->settings->get('config_blog_settings.feed', 1)) {
            $this->journal2->settings->set('blog_blog_feed_url', $this->url->link('journal2/blog/feed', $category_info ? 'journal_blog_feed_category_id=' . $category_id : '', $this->config->get('config_secure')));
        }
        ...
        foreach ($posts as $post) {
            $this->data['posts'][] = array(
                ...
                'href'          => $this->url->link('journal2/blog/post', ($category_info ? 'journal_blog_category_id=' . $category_id . '&' : '') . 'journal_blog_post_id=' . $post['post_id'], $this->config->get('config_secure')),
                ...
                'short_description'   => empty($post['short_description']) ? "" : utf8_substr(strip_tags(html_entity_decode($post['short_description'], ENT_QUOTES, 'UTF-8')), 0, 125) . '...',
                'tags' => $post['tags']
            );
        }
        
        $this->data['tags'] = array();
        $this->data['selected_post_filter'] = empty($tag) ? 'No filter' : $tag;
        $this->data['url_no_filter_tag'] = $this->url->link('journal2/blog', '', $this->config->get('config_secure'));
        $tags = $this->model_journal2_blog->getTags(-1);
        foreach ($tags as $tags_item) {
            $this->data['tags'][] = array(
                'name'  => $tags_item,
                'selected' => ($tag==$tags_item) ? "selected" : "",
                'href'  => $this->url->link('journal2/blog', 'journal_blog_tag=' . $tags_item, $this->config->get('config_secure'))
            );
        }

        $this->data['button_continue'] = $this->language->get('button_continue');
        $this->data['continue'] = $this->url->link('common/home', '', $this->config->get('config_secure'));
        ...
        $pagination->url = $this->url->link('journal2/blog', $url . '&page={page}', $this->config->get('config_secure'));
    ...
    public function post() {
        ...
        if ($category_info) {
            $this->data['breadcrumbs'][] = array(
                'text'      => $category_info['name'],
                'href'      => $this->url->link('journal2/blog', 'journal_blog_category_id=' . $category_id, $this->config->get('config_secure')),
                'separator' => $this->language->get('text_separator')
            );
        }
        ...
        if ($post_info) {
            $this->data['breadcrumbs'][] = array(
                'text'      => $post_info['name'],
                'href'      => $this->url->link('journal2/blog/post', ($category_info ? 'journal_blog_category_id=' . $category_id . '&' : '') . 'journal_blog_post_id=' . $post_info['post_id'], $this->config->get('config_secure')),
                'separator' => $this->language->get('text_separator')
            );
            ...
            foreach (explode(',', $post_info['tags']) as $tag) {
                ...
                if (!$tag) continue;
                $this->data['post_tags'][] = array(
                    'href'  => $this->url->link('journal2/blog', 'journal_blog_tag=' . $tag, $this->config->get('config_secure')),
                    'name'  => $tag
                );
            }
            ...
            foreach ($results as $result) {
                $this->data['post_categories'][] = array(
                    'href'  => $this->url->link('journal2/blog', 'journal_blog_category_id=' . $result['category_id'], $this->config->get('config_secure')),
                    'name'  => $result['name']
                );
            }
            ...
            if ($this->journal2->settings->get('config_blog_settings.related_products', '1')) {
                ...
                foreach ($results as $result) {
                    ...
                    $this->data['related_products'][] = array(
                        ...
                        'short_description' => empty($result['short_description']) ? "" : utf8_substr(strip_tags(html_entity_decode($result['short_description'], ENT_QUOTES, 'UTF-8')), 0, 125) . '...',
                    );
                }
            }
            ...
            $this->document->setDescription($this->blog_meta_description);
            $this->document->setKeywords($this->blog_meta_keywords);
            $this->document->addLink($this->url->link('journal2/blog/post', 'journal_blog_post_id=' . $post_id, $this->config->get('config_secure')), 'canonical');
            $this->journal2->settings->set('blog_meta_title',       $this->blog_meta_title);
...
public function emailVarify($username, $useremail, $comment){
    ...
    public function feed() {
        ...
        $output .= '<channel>';
        $output .= '<atom:link href="' . $this->url->link('journal2/blog', '', $this->config->get('config_secure')) . '" rel="self" type="application/rss+xml" />';
        $output .= '<title>' . $this->blog_title . '</title>';
        $output .= '<link>' . $this->url->link('journal2/blog', '', $this->config->get('config_secure')) . '</link>';
        ...
        foreach ($this->model_journal2_blog->getPosts($data) as $post) {
            ...
            $output .= '<link>' . $this->url->link('journal2/blog/post', 'journal_blog_post_id=' . $post['post_id'], $this->config->get('config_secure')) . '</link>';
            $output .= '<guid>' . $this->url->link('journal2/blog/post', 'journal_blog_post_id=' . $post['post_id'], $this->config->get('config_secure')) . '</guid>';
            ...
            $description .= utf8_substr(strip_tags(html_entity_decode($post['description'], ENT_QUOTES, 'UTF-8')), 0, $this->journal2->settings->get('config_blog_settings.description_char_limit', 150)) . '... ';
            $description .= '<a href="' . $this->url->link('journal2/blog/post', 'journal_blog_post_id=' . $post['post_id'], $this->config->get('config_secure')) . '">' . $this->journal2->settings->get('blog_button_read_more', 'Read More') .'</a>';
