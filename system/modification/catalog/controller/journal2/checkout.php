...
class ControllerJournal2Checkout extends Controller {
    ...
    public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('checkout/cart', '', $this->config->get('config_secure'))
        );
        ...
        if ($this->config->get('config_checkout_id')) {
            ...
            if ($information_info) {
                $this->data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link(Front::$IS_OC2 ? 'information/information/agree' : 'information/information/info', 'information_id=' . $this->config->get('config_checkout_id'), 'SSL'), $information_info['title'], $information_info['title']);  //勾选同意选项框
                //var_dump($this->data['text_agree']);
            } else {
...
$customer_id = ($this->customer->isLogged()) ? (int)$this->customer->getId() : $this->session->data['guest']['email'];
    ...
    public function shipping($return = false) {
        ...
        if (!$this->data['shipping_methods']) {
            $this->data['error_warning'] = sprintf($this->language->get('error_no_shipping'), $this->url->link('information/contact', '', $this->config->get('config_secure')));
        } else {
            $this->data['error_warning'] = '';
        }
    ...
    public function payment($return = false) {
        ...
        if (!$this->data['payment_methods']) {
            $this->data['error_warning'] = sprintf($this->language->get('error_no_payment'), $this->url->link('information/contact', '', $this->config->get('config_secure')));
        } else {
            $this->data['error_warning'] = '';
        }
...
if ($this->config->get('config_cart_weight')) {
    ...
    public function cart_update() {
        ...
        if (!$this->checkCart()) {
            $json['redirect'] = $this->url->link('checkout/cart', '', $this->config->get('config_secure'));
        } else {
    ...
    public function reward() {
        ...
        if (!$json) {
            ...
            if (isset($this->request->post['redirect'])) {
                $json['redirect'] = $this->url->link($this->request->post['redirect'], '', $this->config->get('config_secure'));
            } else {
                $json['redirect'] = $this->url->link('checkout/cart', '', $this->config->get('config_secure'));
            }
        }
    ...
    private function registerAccount() {
        ...
        if ($customer_group_info && !$customer_group_info['approval']) {
            ...
            $redirect = $this->url->link('account/success', '', $this->config->get('config_secure'));
        }
