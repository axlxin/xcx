...
'price_zero'       => $product['price']==0 ? 1 : 0,

                    'special'       => $special,
                    'date_end'      => $date_end,
                    'rating'        => $rating,
                    'description'   => utf8_substr(strip_tags(html_entity_decode($product['description'], ENT_QUOTES, 'UTF-8')), 0, 100) . '..',
                    'short_description'   => empty($product['short_description']) ? "" : utf8_substr(strip_tags(html_entity_decode($product['short_description'], ENT_QUOTES, 'UTF-8')), 0, 250) . '..',
                    'tax'           => $tax,
        ...
        foreach ($sections_data as $section) {
            ...
            foreach ($categories as $category) {
                ...
                $category_data = array(
                    ...
                    'href'          => $this->url->link('product/category', 'path=' . $parent_id . $category_info['category_id'], $this->config->get('config_secure')),
