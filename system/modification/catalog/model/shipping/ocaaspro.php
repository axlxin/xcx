...
if(!$pcb){
	...
	public function getQuote($address, $pcb=false, $pcb_cartProducts=null, $pcb_savedCart=null) {   
		$this->construct($address, $pcb, $pcb_cartProducts, $pcb_savedCart);
		$this->load->language($this->type . '/' . $this->extension); 
		 // $this->writeDebug($address);
	...
	private function getCartTotal() {
		...
		return array(
			0	=> (float)$total,
			1	=> (float)$this->cart->getSubTotal(),
			2	=> (float)$this->cart->getTotal()
		);
	}
