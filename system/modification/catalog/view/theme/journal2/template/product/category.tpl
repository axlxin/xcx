    ...
    <div id="content" class="<?php echo $class; ?>">
      ...
      <div class="product-filter">
        ...
        <div class="filter"><b>Filter by:</b>
            <span class="filter-text" id='category-filter-switch'><?php echo $selected_category_filter; ?></span>
        </div>
        <div class="sort"><b><?php echo $text_sort; ?></b>
          <select onchange="location = this.value;">
            <?php foreach ($sorts as $sorts) { ?>
            <?php if ($sorts['value'] == $kp_sort) { ?>
      ...
      <div class="category-filters-box">
            <a href="<?php echo $url_no_filter; ?>" class='category-filter-item'>No filter</a>
            <?php foreach($category_filters as $category_filter){ ?>
            <a href="<?php echo $category_filter['href']; ?>" class='category-filter-item <?php echo $category_filter["selected"]; ?> '><?php echo $category_filter['name']; ?></a>
            <?php } ?>
      </div>
      ...
      <div class="row main-products product-list" data-grid-classes="<?php echo $this->journal2->settings->get('product_grid_classes'); ?> display-<?php echo $this->journal2->settings->get('product_grid_wishlist_icon_display'); ?> <?php echo $this->journal2->settings->get('product_grid_button_block_button'); ?>">
        ...
        <div class="product-list-item xs-100 sm-100 md-100 lg-100 xl-100">
          <div class="product-thumb <?php echo isset($product['labels']) && is_array($product['labels']) && isset($product['labels']['outofstock']) ? 'outofstock' : ''; ?>">
            ...
            <a class="product-thumb-over" href="index.php?route=product/product&product_id=<?php echo $product['product_id']; ?>">
            <?php }else{ ?>
            <a class="product-thumb-over" href="<?php echo $product['href']; ?>">
            <?php } ?>
                  <h4 class="over-name"><?php echo $product['name']; ?></h4>
                  <div class="short_description" href="<?php echo $product['href']; ?>"><?php echo $product['short_description']; ?></div>
                  <div class="over-product-details">
                      <?php if($product['price_zero']){ ?>
                      <div style="border-radius: 0;margin: 0 auto;" class="button hint--top" target="_blank">Contact&nbsp;&nbsp;Us</div>
                      <?php }else{ ?>
                      <div class="flex-item" style="text-align:left">

<?php if($product['price_zero']){
    echo '<p class="price" style="opacity:0">none</p>';
    $product['price'] = false;
 } ?>
...
<?php if($product['price_zero']){ ?>
    <div class="wishlist" style="opacity:0"><a onclick="javascript:void(0);" class="hint--top" data-hint="<?php echo $button_wishlist; ?>"><i class="wishlist-icon"></i><span class="button-wishlist-text"><?php echo $button_wishlist;?></span></a></div>
    <div class="compare" style="opacity:0"><a onclick="javascript:void(0);" class="hint--top" data-hint="<?php echo $button_compare; ?>"><i class="compare-icon"></i><span class="button-compare-text"><?php echo $button_compare;?></span></a></div>
<?php }else{ ?>
...
<?php if($product['price_zero']){ ?>
    <a style="width:120px;height:35px" class="button hint--top" href="index.php?route=product/product&product_id=<?php echo $product['product_id']; ?>" target="_blank">Contact&nbsp;&nbsp;Us</a>
<?php }else{ ?>
    <a style="height:35px" onclick="addToCart('<?php echo $product['product_id']; ?>');" class="button hint--top" data-hint="<?php echo $button_cart; ?>"><i class="button-left-icon"></i><span class="button-cart-text"><?php echo $button_cart; ?></span><i class="button-right-icon"></i></a>
<?php } ?>
