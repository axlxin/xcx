...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
                if(isset($data['kp_sort']) && $data['kp_sort']=='best_seller'){
                    $sql = "SELECT p.product_id, (SELECT SUM(quantity) AS total FROM " . DB_PREFIX . "order_product op WHERE op.product_id = p.product_id) AS sale_total, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
                }else{
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				if ($implode) {
					...
					$sql .= " OR pd.meta_title LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
				}
                                
                                if (!empty($data['filter_meta_description'])) {
					$sql .= " OR pd.meta_description LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
				}
					...
					$sql .= " OR pd.meta_title LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
                                ...
                                if (!empty($data['filter_misspelling_keyword'])) {
					$sql .= " OR pd.misspelling_keyword LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
				}
		...
		if (!isset($data['kp_sort']) && isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				...
				$sql .= " ORDER BY " . $data['sort'];
                 ...
                 elseif (isset($data['kp_sort']) && !empty($data['kp_sort'])) {if($data['kp_sort']=='best_seller'){$sql .= " ORDER BY sale_total";}else{$sql.= " ORDER BY p.date_added";}} 
    ...
    public function getProductSupper($product_id) {
