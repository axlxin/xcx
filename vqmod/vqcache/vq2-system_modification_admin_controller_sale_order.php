...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		if (substr(VERSION, 0, 1) == 2) {
			...
			$data['link_pdfinv_packing'] = $this->url->link('sale/order/pdf_packingslip', 'token=' . $this->session->data['token'], 'SSL');
			$data['link_pdfinv_backup'] = $this->url->link('sale/order/pdf_backup', 'token=' . $this->session->data['token'], 'SSL');
			...
			$this->data['link_pdfinv_packing'] = $this->url->link('sale/order/pdf_packingslip', 'token=' . $this->session->data['token'], 'SSL');
			$this->data['link_pdfinv_backup'] = $this->url->link('sale/order/pdf_backup', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function info() {
        ...
        $data['pdfinv_ocv21'] = false;
        
        if (substr(VERSION, 0, 3) == '2.1') {
          $data['pdfinv_ocv21'] = true;
        }
				...
				$data['link_pdfinv_packing'] = $this->url->link('sale/order/pdf_packingslip', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
				$data['link_pdfinv_backup'] = $this->url->link('sale/order/pdf_backup', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
				$data['pdf_invoice_manual_backup'] = $this->config->get('pdf_invoice_backup_moment') == 'manual';
				...
				$this->data['link_pdfinv_packing'] = $this->url->link('sale/order/pdf_packingslip', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
				$this->data['link_pdfinv_backup'] = $this->url->link('sale/order/pdf_backup', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
				$this->data['pdf_invoice_manual_backup'] = $this->config->get('pdf_invoice_backup_moment') == 'manual';
...
'usd_price'    		   => $product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0),
...
'usd_text'  => $total['value'],
	...
	public function history() {
		...
		$custom_store_info = isset($this->request->post['custom_store_info']) ? $this->request->post['custom_store_info'] : "";
                ...
                $custom_totals = isset($this->request->post['totals']) ? $this->request->post['totals'] : array();
                ...
                foreach($custom_products as &$product){
                    ...
                    $product['price'] = 'USD' . $this->currency->format($price, 'USD', 1);
                    $product['total'] = 'USD' . $this->currency->format($product_total, 'USD', 1); //把订单使用的货币作为默认货币处理
                }
                
                $order["total"] = array();
                $order["total"][] = array(
                    'title' => 'Sub-Total',
                    'text' => 'USD' . $this->currency->format($subTotal, 'USD', 1)
                );
                $total_total = $subTotal;
                
                foreach($custom_totals as $total){
                    $total_total += preg_replace("/[^0-9.,]+/","",$total['text']);
                    $total['text'] = 'USD' . $this->currency->format(preg_replace("/[^0-9.,]+/","",$total['text']), 'USD', 1);
                    $order["total"][] = $total;
                }
 
                $order["total"][] = array(
                    'title' => 'Total',
                    'text' => 'USD' . $this->currency->format($total_total, 'USD', 1)
                );
