...
SQL;
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', short_description = '" . $this->db->escape($value['short_description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', custom_alt = '" . ((isset($value['custom_alt']))?($this->db->escape($value['custom_alt'])):'') . "', custom_h1 = '" . ((isset($value['custom_h1']))?($this->db->escape($value['custom_h1'])):'') . "', custom_h2 = '" . ((isset($value['custom_h2']))?($this->db->escape($value['custom_h2'])):'') . "', custom_imgtitle = '" . ((isset($value['custom_imgtitle']))?($this->db->escape($value['custom_imgtitle'])):'') . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', misspelling_keyword = '" . $this->db->escape($value['misspelling_keyword']) . "'");
		}
	...
	public function editProduct($product_id, $data) {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
                ...
                if (isset($data['KP_product_option_calculate'])) {
                    $this->load->model('catalog/KP_option_calculate');
            $this->model_catalog_KP_option_calculate->editProductCalculate($product_id,$data['KP_product_option_calculate']);
                }
		...
		foreach ($query->rows as $result) {
			$product_description_data[$result['language_id']] = array(
				...
				'meta_keyword'     => $result['meta_keyword'],
                                'misspelling_keyword'     => $result['misspelling_keyword'],
				'tag'              => $result['tag']
			);
		}
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
                    ...
                    'name'                    => $product_option_value["name"],
						...
						'if_default'                 => $product_option_value['if_default'],
